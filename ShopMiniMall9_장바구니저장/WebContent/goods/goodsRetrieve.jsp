    ...
    <script>
		$(document).ready(function(){
			...
			$("#down").on("click", function(){
				...
				var gAmount = Number.parseInt( $("#gAmount").val());
				if(gAmount == 1){     //최저값인 1이면
					$("#gAmount").val(1);    //값을 1로 고정
				}else{      //1이 아니라면
					$("#gAmount").val(gAmount-1);    // 1씩 줄어들도록
				}
...
<form id="myForm" name="goodRetrieveForm" method="GET" action="#">
		...
		<input type="hidden"type="hidden" name="gCode" value="${goodsRetrieve.gCode}"> 
		<input type="hidden"type="hidden" name="gName" value="${goodsRetrieve.gName}">
		<input type="hidden"type="hidden" name="gPrice" value="${goodsRetrieve.gPrice}">
